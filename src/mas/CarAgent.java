...
public class CarAgent extends Agent {
	...
	protected void setup() {
		...
		if (args != null && args.length > 0) {
			...
			System.out.println(
					getAID().getLocalName() + ": Start is " + start + " vertex, finish is " + finish + " vertex.");
			...
			addBehaviour(new CyclicBehaviour() {
				public void action() {
					...
					if (msg != null) {
						// System.out.println(getAID().getLocalName()+": Reject
						// from road " + msg.getSender().getLocalName());
	...
	private void Request(int numberRoad) {
		// System.out.println(getAID().getLocalName()+": Request to " +
		// roadAgents.get(numberRoad).getLocalName());
	...
	private class SearchBehaviour extends OneShotBehaviour {
		public void action() {
			...
			GraphPath<Integer, DefaultWeightedEdge> gp = DijkstraShortestPath.findPathBetween(City.city, start_,
					finish_);
				...
				for (DefaultWeightedEdge e : listPath) {
					for (int i = 0; i < result.length; ++i) {
						if (result[i].getName().getLocalName()
								.equals("e" + City.city.getEdgeSource(e) + "_" + City.city.getEdgeTarget(e))) {
	...
	public class SeqBehaviour extends SequentialBehaviour {
		...
		public SeqBehaviour(Agent a) {
			...
			addSubBehaviour(new ReceiverBehaviour(myAgent, h, 1000,
